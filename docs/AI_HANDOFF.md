...
- **Mandatory Documentation**: After completing any task or solving a problem, you must automatically update the relevant markdown files (e.g., CHANGELOG.md, PROJECT_STATE.md) to reflect the changes. Do NOT create new markdown files in random locations like the root directory; all documentation must strictly reside in the `docs/` folder.
